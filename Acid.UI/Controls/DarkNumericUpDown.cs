...
namespace Acid.UI.Controls
{
    public class DarkNumericUpDown : NumericUpDown
    {
        ...
        private void RenderArrow(Image image, Rectangle area, PaintEventArgs e)
        {
            Color backColor;
            if (!Enabled)
                backColor = Colours.DarkGreySelection;
            else if (!_mousePos.HasValue || !area.Contains(_mousePos.Value))
                backColor = Colours.LightBackground;
            else if (!_mouseDown)
                backColor = Colours.LighterBackground;
            else
                backColor = Colours.LightestBackground;

            using (Brush brush = new SolidBrush(backColor))
                e.Graphics.FillRectangle(brush, area);
            e.Graphics.DrawImage(image, new Point(area.X + (area.Width - image.Width) / 2, area.Y + (area.Height - image.Height) / 2));
            ControlPaint.DrawBorder(e.Graphics, area, Colours.GreySelection, ButtonBorderStyle.Solid);
        }
